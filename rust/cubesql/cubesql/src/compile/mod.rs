...
impl QueryPlanner {
    ...
    async fn select_to_plan(
        ...
        let result = self
            .create_df_logical_plan(stmt.clone(), qtrace, span_id.clone(), sql_push_down)
            .await?;
    ...
    pub async fn plan(
        ...
        let plan = match (stmt, &self.state.protocol) {
            (ast::Statement::Query(q), _) => {
                if let ast::SetExpr::Select(select) = &q.body {
                    if let Some(into) = &select.into {
                        return self
                            .select_into_to_plan(into, q, qtrace, span_id, sql_push_down)
                            .await;
                    }
                }

                self.select_to_plan(stmt, qtrace, span_id.clone(), sql_push_down)
                    .await
            }
            ...
            (ast::Statement::ShowVariable { variable }, _) => {
                self.show_variable_to_plan(variable, span_id.clone(), sql_push_down)
                    .await
            }
            (ast::Statement::ShowVariables { filter }, DatabaseProtocol::MySQL) => {
                self.show_variables_to_plan(&filter, span_id.clone(), sql_push_down)
                    .await
            }
            ...
            (ast::Statement::ShowCollation { filter }, DatabaseProtocol::MySQL) => {
                self.show_collation_to_plan(&filter, span_id.clone(), sql_push_down)
                    .await
            }
            (ast::Statement::ExplainTable { table_name, .. }, DatabaseProtocol::MySQL) => {
                self.explain_table_to_plan(&table_name, span_id.clone(), sql_push_down)
                    .await
            }
            (
                ...
                self.explain_to_plan(&statement, *verbose, *analyze, sql_push_down)
                    .await
            }
                ...
                && *temporary =>
            {
                ...
                self.create_table_to_plan(name, &stmt, qtrace, span_id.clone(), sql_push_down)
                    .await
            }
            (
                ast::Statement::Drop {
    ...
    async fn show_variable_to_plan(
        ...
        if self.state.protocol == DatabaseProtocol::PostgreSQL {
            ...
            self.create_df_logical_plan(stmt, &mut None, span_id.clone(), sql_push_down)
                .await
...
WHERE `TABLE_SCHEMA` = '{}'",
    ...
    async fn explain_table_to_plan(
        ...
        self.show_columns_to_plan(false, false, &None, table_name, span_id, sql_push_down)
            .await
    }
    ...
    fn explain_to_plan(
        ...
        Box::pin(async move {
            ...
            let plan = self_cloned
                .plan(&statement, &mut None, None, sql_push_down)
                .await?;
    ...
    async fn create_table_to_plan(
        ...
        let plan = self
            .select_to_plan(stmt, qtrace, span_id, sql_push_down)
            .await?;
    ...
    async fn select_into_to_plan(
        ...
        self.create_table_to_plan(&into.name, &new_stmt, qtrace, span_id, sql_push_down)
            .await
    }
    ...
    async fn create_df_logical_plan(
        ...
        let mut converter = LogicalPlanToLanguageConverter::new(cube_ctx.clone(), sql_push_down);
...
pub async fn convert_statement_to_cube_query(
    ...
    let sql_push_down = session.server.config_obj.push_down_pull_up_split();
    planner.plan(&stmt, qtrace, span_id, sql_push_down).await
}
